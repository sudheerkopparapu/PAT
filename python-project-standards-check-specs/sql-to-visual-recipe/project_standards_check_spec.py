...
class ProjectStandardsCheck(ProjectStandardsCheckSpec):
    ...
    def run(self):
        """
        Check if SQL recipes can be converted to visual recipes
        """
        ...
        if not sql_recipes:
            return ProjectStandardsCheckRunResult.not_applicable(
                message =  "There are no sql recipes to check in the project",
                details = details
            )
        ...
        for recipe in sql_recipes:
            ...
            if res["convertible"] == True:
                ...
                convertible_sql_recipes[recipe_name] = link + " " + res.get("explanation")
                lines_of_code_to_save += len(sql_code.split("\n"))

        nbr_convertible_recipes = len(convertible_sql_recipes)
        details["nbr_convertible_recipes"] = nbr_convertible_recipes
        details["lines_of_code_to_save"] = lines_of_code_to_save
        details.update(convertible_sql_recipes)
        error_message = f"{nbr_convertible_recipes} SQL recipe(s) have been identified as convertible to visual recipes."
        ...
        if nbr_convertible_recipes >= critical_threshold:
            ...
            return ProjectStandardsCheckRunResult.success(
                message = f"None of the {len(sql_recipes)} SQL recipes can be converted to visual recipes",
                details = details
            )
