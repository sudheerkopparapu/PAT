...
class ProjectStandardsCheck(ProjectStandardsCheckSpec):
    ...
    def run(self):
        """
        Check that every input, output and share dataset have descriptions
        """  
        ...
        if to_check_dataset_ids:
            for d_id in to_check_dataset_ids:
                ...
                try:
                    ...
                    pass
                if any(["shortDesc" in d.get_settings().get_raw(), "description" in d.get_settings().get_raw()]) == False:
                    for col in columns:
                        if "comment" not in col:
                            to_document_datasets.append(d_id)
                            break
            coverage = 100 - (len(to_document_datasets)/len(to_check_dataset_ids))*100
        else:
            coverage = 100
        
        details["coverage"] = coverage
        details["dataset_to_document"] = md_print_list(to_document_datasets,"dataset",self.original_project_key)

        target_coverage = self.config.get("coverage")
        if coverage >=target_coverage:
            return ProjectStandardsCheckRunResult.success(
                message = f"The project has a {int(coverage)}% dataset description coverage for the following datasets : {', '.join(datasets_to_consider)}",
                details = details
            )
        else:
            return ProjectStandardsCheckRunResult.failure(
                    severity = int(self.config.get("severity")), 
                    message = f"The project has a dataset documentation coverage of {int(coverage)}% which is under the target of coverage of {int(target_coverage)}%. \nAdd Data dataset descriptions to : {md_print_list(to_document_datasets,'dataset',self.original_project_key)}",
                    details = details
                )
